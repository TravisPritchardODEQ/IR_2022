...
# WS Unit ---------------------------------------------------------------------------------------------------------
  ...
  shell_harvest_analysis_WS <- shell_harvest %>%
    ...
    mutate(IR_category = case_when(!is.na(median) & median > median_crit ~ "5",
                                   ...
                                   TRUE ~ "ERROR"),
           Rationale = case_when(!is.na(median) & median > median_crit ~ paste0(MLocID, ": Median fecal coliform concentration of ",
                                                                                median, "/100ml is greater than criteria value of ",
                                                                                median_crit, "- ", num_samples, " total samples" ),
                                   num_samples >= 5 & percent_SS_exceed > 0.1 ~  paste0(MLocID, ": ", percent_SS_exceed, " of all samples exceed single sample criteria value of ",
...
# Other unit ------------------------------------------------------------------------------------------------------
  ...
  shell_harvest_analysis_other <- shell_harvest %>%
    ...
    mutate(IR_category = case_when(!is.na(median) & median > median_crit ~ "5",
                                   ...
                                   num_samples >= 5 & median <= median_crit & num_SS_exceed/num_samples <= 0.1 ~ "2",
                                   TRUE ~ "ERROR"),
           Rationale = case_when(!is.na(median) & median > median_crit ~ paste0("Median fecal coliform concentration of ",
                                                                                median, "/100ml is greater than criteria value of ",
                                                                                median_crit, "- ", num_samples, " total samples" ),
                                 num_samples >= 5 & percent_SS_exceed > 0.1 ~  paste0(percent_SS_exceed, " of all samples exceed single sample criteria value of ",
                                                                                      SS_Crit, " - ", num_samples, " total samples") ,
