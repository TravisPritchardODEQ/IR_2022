  ...
  penta_data_summary <- penta_data_analysis %>%
    ...
    mutate(critical_excursions = binomial_excursions(num_samples, type = 'Toxics'),
           IR_category = case_when(percent_3d == 100 ~ "3D",
                                   ...
                                   TRUE ~ "ERROR"),
           Rationale =  case_when(percent_3d == 100 ~ paste0("All results are non-detects with detection limits above criteria- ",
                                                                                  num_samples, " total samples"),
                                  num_samples < 2 & num_excursions >= critical_excursions ~  paste0(num_excursions,
                                                                                                    " excursion of criteria with ",
                                                                                                    num_samples, " total samples. ", 
                                                                                                    critical_excursions, " needed to list."),
                                  num_samples < 10 & num_excursions == 0 ~ paste0("Only ",num_samples, " sample with ",
                                                                                  num_excursions," excursions"),
                                  num_samples < 10 & num_excursions > 0 ~ paste0("Only ",num_samples, " sample with ",
                                                                                  num_excursions," excursion"),
                                  
                                  num_samples >=  10 & num_excursions < critical_excursions ~ paste0(num_excursions,
                                                                                  " excursion of criteria with ",
                                                                                  num_samples, " total samples. ", 
                                                                                  critical_excursions, " needed to list."),
                                  TRUE ~ "ERROR"))
